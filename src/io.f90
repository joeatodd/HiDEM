...
CONTAINS
 ...
 SUBROUTINE ReadInput(INFILE, SimInfo)
   ...
   IF(myid==0) THEN
     ...
     WRITE(*,'(A,ES12.5)') "Viscous Distance = ", SimInfo % ViscDist
     WRITE(*,'(A,ES12.5)') "Viscous Strength = ", SimInfo % ViscStrength
...
SUBROUTINE BinarySTROutput(SI,NRY,NTOT,NANPart,EFS,MAXV2,TENS)
  ...
  lfeed = CHAR(10) !line feed character
  NVars = 3 !EFS    - STR, XYZ obtained using .vtu file
  ...
  DO j=1,NTOT
    IF(NANPart(j) < myid) CYCLE
    counter = counter + 1
    work_real(counter*3-2) = EFS(j)
    work_real(counter*3-1) = MAXV2(j)
    work_real(counter*3-0) = TENS(j)
  END DO
