...
label_attr_map = {
       "front_orientation:": ["front_orientation", float, float, float],
        ...
        "xmin_cut:": [ "xmin_cut", float],
        "xmax_cut:": [ "xmax_cut", float],
        "ymin_cut:": [ "ymin_cut", float],
        "ymax_cut:": [ "ymax_cut", float],
...
#Possibility to chop parts of the domain off (e.g. to ensure a free edge of fjord)
if(hasattr(params,"xmax_cut")):
    xmax_cut = params.xmax_cut
else:
    xmax_cut = 0.0

if(hasattr(params, "xmin_cut")):
    xmin_cut = params.xmin_cut
else:
    xmin_cut = 0.0

if(hasattr(params,"ymax_cut")):
    ymax_cut = params.ymax_cut
else:
    ymax_cut = 0.0

if(hasattr(params,"ymin_cut")):
    ymin_cut = params.ymin_cut
else:
    ymin_cut = 0.0
...
outfile = os.path.basename(result_file)
outfile = outfile.replace(extension,"dat")

transform_outfile = os.path.basename(result_file)
transform_outfile = transform_outfile.replace(extension,"transform")
...
if(determine_transform):
	...
	x_max = clip_points[:,0].max() + x_translate
        ...
        x_min = x_min - grid_buff + xmin_cut
        x_max = x_max + grid_buff - xmax_cut

        y_min = 0.0 + ymin_cut
        y_max = upstream_extent + downstream_extent + grid_buff - ymax_cut #buffer downstream end of domain
	...
	with open(transform_file, 'r') as transformin:
		...
		dummy_line = transformin.readline()
                #read extra line if new format
                if 'DAT MD5' in dummy_line:
		    transformin.readline()
